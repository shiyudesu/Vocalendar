...
| API 通用规范、全量预定义接口契约与核心对象     | `docs/api-spec.md`     |
| 单模块范围、流程、验收、节奏与模块内接口决策 | `docs/modules/*.md`    |
| 仓库协作与同步规则                           | `AGENTS.md`            |
...
- 开发工作必须围绕一个明确模块进行
- 如果当前任务不属于任何已定义模块，先更新 `docs/prd.md` 和对应模块 plan，再开始实现
- 对尚未进入开发的模块，不提前编写完整 plan；在准备启动该模块时再创建
- 不允许把后续模块范围偷偷塞进当前模块

### 4.2 文档同步

- 修改产品边界或版本目标：更新 `docs/prd.md`
- 修改当前模块的接口决策或数据结构：先更新对应 `docs/modules/*.md`
- API 通用规范或全量预定义接口契约发生变化：同步更新 `docs/api-spec.md`
- 修改技术结构或工程约束：更新 `docs/architecture.md`
- 修改当前模块流程、验收或交付内容：更新对应 `docs/modules/*.md`

如果一次代码改动影响多个维度，必须在同一个变更中同步更新相关文档。

### 4.3 先计划后实现

- 新功能、较大重构或跨文件改动，应先确认对应模块 plan 已存在
- 如果 plan 不存在或明显过期，先更新模块计划文档
- 只有在计划和边界清晰后，才进入代码实现

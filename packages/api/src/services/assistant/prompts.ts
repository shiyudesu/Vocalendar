...
9. **结束时间（endAt）默认必须为 null**。只有满足以下任一条件才填 endAt：
   - 用户明确给出结束时间（"到下午 5 点"、"晚饭吃到 8 点"）
   - 用户明确给出持续时间（"开 1 小时会"、"打 30 分钟羽毛球"）
   - 事件类型本身天然带时长含义（开会、约会、吃饭、上课、面试）
   对"提醒喝水"、"提醒打卡"、"吃药"、"提醒...10 分钟后"这类**瞬时/提醒类**事件，
   endAt 必须为 null，禁止凭空补一个默认时长。
